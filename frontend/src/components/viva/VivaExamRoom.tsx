...
import { Mic, MicOff, Volume2, VolumeX, Loader2, CheckCircle2, XCircle, RotateCcw, Trophy, AlertCircle, Eye } from 'lucide-react';
...
export function VivaExamRoom({ sessionId, moduleTitle, userGoal, onComplete, onRetake }: VivaRoomProps) {
  ...
  // Proctoring state
  const [violations, setViolations] = useState<Array<{type: string; message: string; timestamp: Date}>>([]);
  ...
  const handleViolation = (type: string, message: string) => {
    const violation = { type, message, timestamp: new Date() };
  ...
  if (!isComplete) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 p-4 flex gap-4">
        {/* Proctoring Panel */}
        <div className="w-80 shrink-0 space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              <ProctorEye onViolation={handleViolation} isActive={proctorActive} />
              <AudioProctor 
                onViolation={handleViolation} 
                isAiSpeaking={isSpeaking}
                isUserAnswering={isListening}
              />
              
              {violations.length > 0 && (
                <div className="mt-4">
                  <p className="text-xs font-medium mb-2">Violations: {violations.length}</p>
