"""
Viva Voce Agent - 100% FREE Implementation
Uses Gemini for conversation, Groq for STT, Edge-TTS for TTS
"""
import json
import os
import asyncio
...
# Allow nested event loops (needed for FastAPI + edge-tts)
nest_asyncio.apply()
...
class VivaAgent:
    ...
    100% FREE: Gemini + Groq + Edge-TTS
    """
    
    def __init__(self):
        """Initialize the Viva Agent with free services."""
        ...
        # Edge-TTS for text-to-speech (FREE - Microsoft)
        # No API key needed!
        self.tts_voice = "en-US-AriaNeural"  # Professional female voice
        # Alternative voices:
...
Keep it professional, motivating, and conversational. Output ONLY the feedback text, no formatting."""
    ...
    def transcribe_audio(self, audio_file) -> str:
        """
        Transcribe audio to text using Groq (Whisper-v3).
        ...
        try:
            ...
            transcription = self.groq_client.audio.transcriptions.create(
                file=audio_file,
                model="whisper-large-v3-turbo",  # Ultra-fast, free
                language="en",
